...
public class StudentXMLRepository extends AbstractXMLRepository<String, Student> {
    ...
    protected Element getElementFromEntity(Student student, Document XMLdocument) {
        ...
        element.appendChild(createElement(XMLdocument, "Email", String.valueOf(student.getEmail())));
        element.appendChild(createElement(XMLdocument, "NameOfProfessor", String.valueOf(student.getNameOfTheProfessor())));
    ...
    protected Student getEntityFromNode(Element node) {
        ...
        String profName = node.getElementsByTagName("NameOfProfessor").item(0).getTextContent();

        return new Student(ID, nume, grupa, email, profName);
    }
}
